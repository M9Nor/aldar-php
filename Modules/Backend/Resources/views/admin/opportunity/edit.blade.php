...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>  __('backend::opportunity.update_opportunity'),
            'items' => [
                [
                    'label' => __('backend::opportunity.title'),
                    'link'  => route('OpportunityController@index')
                ], [
                    'label' => __('backend::opportunity.update_opportunity'),
                    'link'  => 'javascript:;'
                ]
            ]
        ]
                ...
                <div class="dropdown-menu dropdown-menu-{{ $langDirection == 'rtl' ? 'left' : 'right' }}">
                    <ul class="kt-nav">
                        <li class="kt-nav__item">
                            <a href="javascript:;" data-redirect-url="{{ route('OpportunityController@create') }}" class="submit_form kt-nav__link">
                        ...
                        <li class="kt-nav__item">
                            <a href="javascript:;" data-redirect-url="{{ route('OpportunityController@index') }}" class="submit_form kt-nav__link">
...
@section('content')
    <div class="kt-portlet kt-portlet--tabs kt-portlet--last kt-portlet--responsive-mobile" id="kt_page_portlet">
                ...
                <ul class="nav nav-tabs nav-tabs-space-xl nav-tabs-line nav-tabs-bold nav-tabs-line-3x nav-tabs-line-brand" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" data-toggle="tab" href="#add_tab_1" role="tab" aria-selected="false">
                            ...
                            </svg><!--end::Svg Icon--></span>{{ __('backend::opportunity.sections.account.title') }}
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#add_tab_2" role="tab" aria-selected="false">
                            ...
                            </svg> {{ __('backend::opportunity.sections.profile.title') }}
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#add_tab_3" role="tab" aria-selected="false">
                            ...
                            </svg><!--end::Svg Icon--></span>{{ __('backend::opportunity.sections.location.title') }}
                        </a>
                    </li>
                    <li class="nav-item">
                        ...
                        </svg> {{ __('backend::opportunity.sections.media.title') }}
                        </a>
                    </li>
                   <!--  <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#add_tab_5" role="tab" aria-selected="false">
                           <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                            ...
                            </svg> {{ __('backend::opportunity.sections.service.title') }}
                        </a>
                    </li>
                    <li class="nav-item">
                        ...
                        </svg>{{ __('backend::opportunity.sections.paying.title') }}
                        </a>
                    </li> -->
                    <li class="nav-item">
                        ...
                        </svg>{{ __('backend::opportunity.sections.price.title') }}
                        </a>
                    </li>
                </ul>
            </div>
        ...
        <div class="kt-portlet__body">
            <div class="row">
                <div class="col-md-12">
                    <form onsubmit="onFormSubmit(event);" class="kt-form" id="addNewForm" action="{{ route('OpportunityController@update',['model' => $model->id]) }}" method="POST" enctype="multipart/form-data">
                        <div class="tab-content">
                            ...
                            <div class="tab-pane active" id="add_tab_1" role="tabpanel">
                                ...
                                @include('backend::admin.opportunity.update.main')

                            </div>
                            <div class="tab-pane" id="add_tab_2" role="tabpanel">
                                @include('backend::admin.opportunity.update.details')

                            </div>
                            <div class="tab-pane" id="add_tab_3" role="tabpanel">
                                @include('backend::admin.opportunity.update.location')
                            </div>
                            <div class="tab-pane" id="add_tab_4" role="tabpanel">
                                @include('backend::admin.opportunity.update.media')
                            </div>
                            <!-- <div class="tab-pane" id="add_tab_5" role="tabpanel">
                                @include('backend::admin.opportunity.update.service')
                            </div>
                            <div class="tab-pane" id="add_tab_6" role="tabpanel">
                                @include('backend::admin.opportunity.update.pay')
                            </div> -->
                            <div class="tab-pane" id="add_tab_7" role="tabpanel">
                                @include('backend::admin.opportunity.update.price')
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    $(".delete-payment").click(function() {
        ...
        swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ route('OpportunityController@deletePayment')}}",
                    type: "POST",
                    data: {
                    ...
                    success: function(response) {
                        if(response.success){
                            $('#delete_payment_'+ payment_id).remove();
                            show_toastr(
                                response.type,
                                response.title,
    ...
    $(".delete-price").click(function() {
        ...
        swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ route('OpportunityController@deletePrice')}}",
                    type: "POST",
                    data: {
                        price_id : price_id,
                        _token: "{{ csrf_token() }}"
