...
@include('backend::admin.opportunity.filter')
...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>  __('backend::opportunity.title'),
            'items' => [
                [
        ...
        @slot('toolbar')
            ...
            <form id="massDeleteForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('OpportunityController@massDestroy') }}" style="display: none;">
            ...
            <form id="massRestoreForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('OpportunityController@massRestore') }}" style="display: none;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <input type="hidden" name="ids">
            </form>
            ...
            @if(auth()->user()->can('create', Project::class))
                <a href="{{ route('OpportunityController@create') }}" class="btn btn-outline-success btn-sm btn-icon btn-icon-md btn-elevate datatable-custom-tools" data-toggle="kt-tooltip" data-placement="right" title="" data-original-title="{{ __('cms::global.add_new') }}">
    ...
    @component('cms::components.partials.portlet')
        ...
        <table class="table table-bordered table-striped table-hover table-checkable" id="datatable">
            <thead>
                <tr class="cms-font">
                    <th width="5%"></th>
                    <th width="35%">{{ __('backend::opportunity.title') }}</th>
                    <th width="10%"> {{__('backend::opportunity.fields.code.label')}} </th>
                    <th width="15%">{{ __('cms::cruds.contents.languages') }}</th>
...
@push('scripts')
    <script>
        ...
        $(function() {
            $.extend(options, {
                ajax: {
                    url: "{!! route('OpportunityController@data') !!}",
                    type: "GET",
                    data : function (d) {
                        d._token    = '{!! csrf_token() !!}';
                                ...
                                <a href="javascript:;">
                                    <div class="kt-user-card-v2">
                                        ...
                                        <div class="kt-user-card-v2__details">
                                            <span class="kt-user-card-v2__name">${row.translated_name}</span>
                                        </div>
                                    </div>
                                </a>
                            `;
            ...
            @if(auth()->user()->can('viewDeleted', \Modules\Backend\Entities\Project::class))
