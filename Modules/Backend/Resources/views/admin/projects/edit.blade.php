...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>  __('backend::projects.update_project'),
            'items' => [
                [
                    'label' => __('backend::projects.title'),
                    'link'  => route('ProjectController@index')
                ], [
                    'label' => __('backend::projects.update_project'),
                    'link'  => 'javascript:;'
                ]
            ]
        ]
...
@section('content')
    <div class="kt-portlet kt-portlet--tabs kt-portlet--last kt-portlet--responsive-mobile" id="kt_page_portlet">
        ...
        <div class="kt-portlet__body">
            <div class="row">
                <div class="col-md-12">
                    <form onsubmit="onFormSubmit(event);" class="kt-form" id="addNewForm" action="{{ route('ProjectController@update',['model' => $model->id]) }}" method="POST" enctype="multipart/form-data">
                        <div class="tab-content">
                            ...
                            <div class="tab-pane active" id="add_tab_1" role="tabpanel">
                                ...
                                @include('backend::admin.projects.update.main')

                            </div>
                            <div class="tab-pane" id="add_tab_2" role="tabpanel">
                                @include('backend::admin.projects.update.details')

                            </div>
                            <div class="tab-pane" id="add_tab_3" role="tabpanel">
                                @include('backend::admin.projects.update.location')
                            </div>
                            <div class="tab-pane" id="add_tab_4" role="tabpanel">
                                @include('backend::admin.projects.update.media')
                            </div>
                            <div class="tab-pane" id="add_tab_5" role="tabpanel">
                                @include('backend::admin.projects.update.service')
                            </div>
                            <div class="tab-pane" id="add_tab_6" role="tabpanel">
                                @include('backend::admin.projects.update.pay')
                            </div>
                            <div class="tab-pane" id="add_tab_7" role="tabpanel">
                                @include('backend::admin.projects.update.price')
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    $(".delete-payment").click(function() {
        ...
        swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ route('ProjectController@deletePayment')}}",
                    type: "POST",
                    data: {
    ...
    $(".delete-price").click(function() {
        ...
        swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ route('ProjectController@deletePrice')}}",
                    type: "POST",
                    data: {
                        price_id : price_id,
                        _token: "{{ csrf_token() }}"
