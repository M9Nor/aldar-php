...
@include('backend::admin.projects.filter')
...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>  __('backend::projects.title'),
            'items' => [
                [
        ...
        @slot('toolbar')
            ...
            <form id="massDeleteForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('ProjectController@massDestroy') }}" style="display: none;">
            ...
            <form id="massRestoreForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('ProjectController@massRestore') }}" style="display: none;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <input type="hidden" name="ids">
            </form>
            ...
            @if(auth()->user()->can('create', Project::class))
                <a href="{{ route('ProjectController@create') }}" class="btn btn-outline-success btn-sm btn-icon btn-icon-md btn-elevate datatable-custom-tools" data-toggle="kt-tooltip" data-placement="right" title="" data-original-title="{{ __('cms::global.add_new') }}">
    ...
    @component('cms::components.partials.portlet')
        ...
        <table class="table table-bordered table-striped table-hover table-checkable" id="datatable">
            <thead>
                <tr class="cms-font">
                    <th width="5%"></th>
                    <th width="35%">{{ __('backend::projects.title') }}</th>
                    <th width="10%"> {{__('backend::projects.fields.code.label')}} </th>
...
@push('scripts')
    <script>
        ...
        $(function() {
            $.extend(options, {
                ajax: {
                    url: "{!! route('ProjectController@data') !!}",
                    type: "GET",
                    data : function (d) {
                        d._token    = '{!! csrf_token() !!}';
