    ...
    @component('cms::components.partials.portlet')
        ...
        <table class="table table-bordered table-striped table-hover table-checkable" id="datatable">
            <thead>
                <tr class="cms-font">
                    <th></th>
                    <th>{{ __('backend::projects.datatable.project') }}</th>
                    <th>{{ __('backend::projects.datatable.location') }}</th>
                    <th>{{ __('backend::projects.datatable.status') }}</th>
                    <th width="5%">{{ __('backend::projects.datatable.actions') }}&nbsp;&nbsp;&nbsp;&nbsp;</th>
                </tr>
            </thead>
        </table>
    @endcomponent
    {{-- @include('cms::admin.projects.modal') --}}
@endsection
...
@push('scripts')
    <script>
        ...
        $(function() {
            $.extend(options, {
                ajax: {
                    url: "{!! is_null(request()->type) ? route('ProjectController@data') : route('ProjectController@data', ['type' => mb_strtolower(request()->type)]) !!}",
                    type: "GET",
                    data : function (d) {
                        d._token    = '{!! csrf_token() !!}';
                ...
                headerCallback: function(thead, data, start, end, display) {
                    thead.getElementsByTagName('th')[0].innerHTML = `
                        <label class="kt-checkbox kt-checkbox--single kt-checkbox--solid kt-checkbox--brand">
                            ...
                            <span></span>{{ __('backend::projects.datatable.id') }}
                        </label>`;
                },
                columns: [
                    {
