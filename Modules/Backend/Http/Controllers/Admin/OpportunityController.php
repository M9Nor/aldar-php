...
class OpportunityController extends CmsController
{
     ...
     public function index(Request $request)
    {
        ...
        $this->data['projectCategories']        = Category::with('translations')->whereIn('type',['contracts','opportunity_classifications','property_status','property_features','facilities','payments'])->get();
        $this->data['contracts']                = clone $this->data['projectCategories']->where('type','contracts');
        $this->data['opportunity_classifications'] = clone $this->data['projectCategories']->where('type','opportunity_classifications')->whereNull('parent_id');
        $this->data['property_type']            = clone $this->data['projectCategories']->where('type','opportunity_classifications')->whereNotNull('parent_id');
        ...
        return view('backend::admin.opportunity.index', $this->data);
    }
    public function data(Request $request)
    {
        ...
        $list = CrudModel::select(['be_projects.*',
            ...
            ')
        ])->with(['translations','allCategories.translations','allCategories'])->whereHas('allCategories', function ($query) {
    $query->where('type', 'opportunity_classifications');
        ...
        ->addIndexColumn() // Adds an incremental first row.
         ->filter(function($q) use ($request) {
              if(!empty($filter = $request->filter) && is_array($filter)){
                ...
                })->when(!empty($filter['opportunity_classifications']), function($query) use ($filter) {
                    $query->whereHas('categories',function($q) use ($filter){
                        $q->where('id',$filter['opportunity_classifications']);
                    });
        ...
        ->addColumn('translated_name', function($model) {
            ...
            return '<a style="font-weight: bold; color: #595d6e;" target="_blank" rel="noopener" rel="noopener" href="'.route('OpportunityController@single', ['type' => (!is_null($category) ? $category->slug : 'unknown'), 'slug' => $model->slug]).'">'.$new_title.'</a>';
            ...
            foreach($model->translations->sortBy('locale') as $translation)
            {
                ...
                if(auth()->user()->can('deleteTranslation', $model) && $translation->locale != 'ar')
                {
                    $output .= '<div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end">
                        ...
                        <form id="deleteTranslationForm'.$model->id.'" onsubmit="onFormSubmit(event);" method="POST" action="'.route('OpportunityController@destroyTranslation', ['model' => $model->id, 'locale' => $translation->locale]).'" style="display: none;">
            ...
            if(!$model->trashed())
            {
                if(auth()->user()->can('update', $model))
                {
                    $items[] = array_merge($this->actions['edit'], [
                        'url'   => route('OpportunityController@edit', ['model' => $model->id]),
                        'id'    => 'edit_' . $model->id
                    ]);
                }
                if(auth()->user()->can('delete', $model))
                {
                    $items[] = array_merge($this->actions['delete'], [
                        'url'   => route('OpportunityController@destroy', ['model' => $model->id]),
                        'id'    => 'delete_' . $model->id
                    ]);
                }
                if(is_null($model->disabled_at))
                {
                    if(auth()->user()->can('disable', $model)){
                        $items[] = array_merge($this->actions['disable'], [
                            'url'   => route('OpportunityController@disable', ['model' => $model->id]),
                    ...
                    if(auth()->user()->can('enable', $model)){
                        $items[] = array_merge($this->actions['enable'], [
                            'url'   => route('OpportunityController@enable', ['model' => $model->id]),
                            'id'    => 'enable_' . $model->id
                        ]);
                    }
                }
                if(is_null($model->is_special) || $model->is_special == 0)
                {
                    if(auth()->user()->can('special', $model)){
                        $items[] = array_merge($this->actions['special'], [
                            'url'   => route('OpportunityController@special', ['model' => $model->id]),
                    ...
                    if(auth()->user()->can('not_special', $model)){
                        $items[] = array_merge($this->actions['not_special'], [
                            'url'   => route('OpportunityController@notSpecial', ['model' => $model->id]),
                ...
                if(is_null($model->is_sold) || $model->is_sold == 0)
                {
                    if(auth()->user()->can('update', $model)){
                        $items[] = array_merge($this->actions['sold'], [
                            'url'   => route('OpportunityController@sold', ['model' => $model->id]),
                            'id'    => 'sold_' . $model->id
                        ]);
                    }
                }else{
                    if(auth()->user()->can('update', $model)){
                        $items[] = array_merge($this->actions['not_sold'], [
                            'url'   => route('OpportunityController@notSold', ['model' => $model->id]),
                ...
                if(auth()->user()->can('update', $model)){
                    $items[] = array_merge($this->actions['copy'], [
                        'url'   => route('OpportunityController@copy', ['model' => $model->id]),
                        'id'    => 'copy_' . $model->id
                    ]);
                }
            }
            if($model->trashed())
            {
                if(auth()->user()->can('restore', $model))
                {
                    $items[] = array_merge($this->actions['restore'], [
                        'url'   => route('OpportunityController@restore', ['model' => $model->id]),
    ...
    public function create(Request $request)
    {
        ...
        // $this->data['opportunity_classifications'] = $this->data['projectCategories']->whereNull('parent_id')->where('type','opportunity_classifications');
        ...
        return view('backend::admin.opportunity.create', $this->data);
    }
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            'about_ar'              => 'required|string|max:20000',
            ...
            'roi'                   => 'nullable|digits_between:1,9|numeric|max:999999999|min:1',
        ];
        foreach(LaravelLocalization::getSupportedLocales() as $locale => $lang){
            if($locale != 'ar'){
                if(!is_null($request->input('title_'.$locale))){
                    ...
                    $rules['description_'.$locale]      = 'nullable|string|max:20000';
                    $rules['details_'.$locale]          = 'nullable|string|max:20000';
                    $rules['about_'.$locale]            = 'required|string|max:20000';
                    $rules['brief_'.$locale]            = 'nullable|string|max:1000';
        ...
        if(!empty($request->prices)){
            ...
            $rules['prices.*.room']             = 'nullable|numeric|max:999';
            $rules['prices.*.salon']            = 'nullable|numeric|max:999999999';
            // $rules['prices.*.bath']             = 'nullable|numeric|max:999999999|min:0';
            $rules['prices.*.lowest_area']      = 'required|numeric|max:999999999|min:0';
            $rules['prices.*.highest_area']     = 'nullable|numeric|max:999999999|min:0';
            $rules['prices.*.lowest_price']     = 'required|numeric|max:999999999|min:0';
        ...
        try {
            DB::transaction(function() use ($request) {
                ...
                if(!is_null($request->input('roi'))){
                $this->data['model']->roi                   = $request->roi;
                }
    ...
    public function edit(Request $request)
    {
        $this->data['model']                    = CrudModel::withDisabled()->with([
            'categories' => function($q){
                $q->CategoryType(['contracts','opportunity_classifications','property_status','property_features','facilities','payments']);
            },
            'contents' => function($q){
        ...
        return view('backend::admin.opportunity.edit', $this->data);
    }
    public function update(Request $request)
    {
        ...
        if(!empty($request->new_prices)){
            ...
            $rules['new_prices.*.room']             = 'nullable|numeric|max:999';
            $rules['new_prices.*.salon']            = 'nullable|numeric|max:999999999';
            // $rules['new_prices.*.bath']             = 'nullable|numeric|max:999999999|min:0';
            $rules['new_prices.*.lowest_area']      = 'required|numeric|max:999999999|min:0';
            $rules['new_prices.*.highest_area']     = 'nullable|numeric|max:999999999|min:0';
            $rules['new_prices.*.lowest_price']     = 'required|numeric|max:999999999|min:0';
        ...
        try {
            DB::transaction(function() use ($request) {
                ...
                if(!empty($projectContents)){
                    ...
                    $this->data['model']->contents()->detach($request->agent);
                }
                // save project areas
                if (!empty($request->areas)) {
                    $this->data['model']->areas()->sync($request->areas);
    ...
    public function data_properties(Request $request)
    {
       ...
       ->addColumn('link', function($model){
        ...
        ->addColumn('actions', function($model){
            ...
            // Gives the ROOT user the ability to login with any other user without a password.
            $items[] = array_merge($this->actions['edit'], [
                'url'   => route('OpportunityController@showDetails', ['model' => $model->id]),
                'id'    => 'edit_' . $model->id
            ]);
            if(count($items) > 1)
            {
    ...
    public function showDetails(Request $request)
    {
        ...
        return view('backend::admin.opportunity.show_details', $this->data);
