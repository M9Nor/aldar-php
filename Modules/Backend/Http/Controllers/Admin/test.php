...
class ProjectController extends CmsController
{
    ...
    public function data(Request $request)
    {
        ...
        ->filter(function($q) use ($request) {
            if(!empty($filter = $request->filter) && is_array($filter)){
                ...
                $filter->where('name', 'property_features[]')->map(function($item) use ($propertyFeatures) {
                    $propertyFeatures->push($item['value']);
                    return $item;
                });
                ...
                })->when(!empty($propertyFeatures->toArray()), function($query) use ($filter) {
                    $query->whereHas('categories',function($q) use ($filter){
                        $q->whereIn('id',$filter['property_features']);
                    });
                ...
                })->when(!empty($filter['bath']), function($query) use ($filter) {
                    $query->whereHas('prices',function($q) use ($filter){
                        $q->where('bathes_number',$filter['bath']);
                    });
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            'details_ar'            => 'nullable|string|max:5000',
            'about_ar'              => 'nullable|string|max:5000',
            ...
            'tags'                  => 'nullable|array',
            'tags.*'                => 'required|digits_between:1,9|numeric|max:9999999999|min:1',
            ...
            'agent'                 => 'required|digits_between:1,9|numeric|max:9999999999|min:1',
            'categories_id'         => 'required|digits_between:1,9|numeric|max:9999999999|min:1',
            'project_type'          => 'required|array',
            'project_type.*'        => 'required|digits_between:1,9|numeric|max:9999999999|min:1',
            'offers'                => 'required|array',
            'offers.*'              => 'required|digits_between:1,9|numeric|max:9999999999|min:1',
            'status'                => 'required|array',
            'status.*'              => 'required|digits_between:1,9|numeric|max:9999999999|min:1',
            'features'              => 'required|array',
            'features.*'            => 'required|digits_between:1,9|numeric|max:9999999999|min:1',
            'top_features'          => 'required|array',
            'top_features.*'        => 'required|digits_between:1,9|numeric|max:9999999999|min:1',
            ...
            'pro_area'              => 'nullable|digits_between:1,9|numeric|max:9999999999|min:1',
            'areas'                 => 'nullable|array',
            'areas.*'               => 'nullable|digits_between:1,9|numeric|max:9999999999|min:1',
            ...
            'facilities.*'          => 'required|digits_between:1,9|numeric|max:9999999999|min:1',
        ...
        foreach(LaravelLocalization::getSupportedLocales() as $locale => $lang){
            if($locale != 'ar'){
                if(!is_null($request->input('title_'.$locale))){
                    ...
                    $rules['description_'.$locale]      = 'required|string|max:5000';
                    $rules['details_'.$locale]          = 'required|string|max:5000';
                    $rules['about_'.$locale]            = 'required|string|max:5000';
                    $rules['brief_'.$locale]            = 'required|string|max:1000';
                    $rules['seo_description_'.$locale]  = 'nullable|string|max:5000';
        ...
        if(!empty($request->paying)){
            $rules['paying.*.pay_id']           = 'required|digits_between:1,9|numeric|max:9999999999|min:1';
            $rules['paying.*.first_pay']        = 'required|string|max:191';
            $rules['paying.*.number_pays']      = 'required|digits_between:1,9|numeric|max:9999999999|min:1';
        ...
        if(!empty($request->prices)){
            $rules['prices.*.balance']          = 'required|digits_between:1,9|numeric|max:9999999999|min:1';
            $rules['prices.*.room']             = 'required|between:0,9.9';
            $rules['prices.*.salon']            = 'required|numeric|max:9999999999|min:1';
            $rules['prices.*.bath']             = 'required|numeric|max:9999999999|min:1';
            $rules['prices.*.lowest_area']      = 'required|numeric|max:9999999999|min:1';
            $rules['prices.*.highest_area']     = 'required|numeric|max:9999999999|min:1';
            $rules['prices.*.lowest_price']     = 'required|numeric|max:9999999999|min:1';
            $rules['prices.*.highest_price']    = 'required|numeric|max:9999999999|min:1';
        }
    ...
    public function update(Request $request)
    {
        ...
        if(!empty($request->new_paying)){
            $rules['new_paying.*.pay_id']           = 'required|digits_between:1,9|numeric|max:9999999999|min:1';
            $rules['new_paying.*.first_pay']        = 'required|string|max:191';
            $rules['new_paying.*.number_pays']      = 'required|digits_between:1,9|numeric|max:9999999999|min:1';
        ...
        if(!empty($request->new_prices)){
            $rules['new_prices.*.balance']          = 'required|digits_between:1,9|numeric|max:9999999999|min:1';
            $rules['new_prices.*.room']             = 'required|numeric|max:9999999999|min:1';
            $rules['new_prices.*.salon']            = 'required|numeric|max:9999999999|min:1';
            $rules['new_prices.*.bath']             = 'required|numeric|max:9999999999|min:1';
            $rules['new_prices.*.lowest_area']      = 'required|numeric|max:9999999999|min:1';
            $rules['new_prices.*.highest_area']     = 'required|numeric|max:9999999999|min:1';
            $rules['new_prices.*.lowest_price']     = 'required|numeric|max:9999999999|min:1';
            $rules['new_prices.*.highest_price']    = 'required|numeric|max:9999999999|min:1';
        }
        $validator = Validator::make($request->all(), $rules, [], $this->attributeNames);
        if($validator->fails()){
            return new ResponseHandler([
                ...
                if($request->paying){
                    foreach($request->paying as $key=>$item){
                        ...
                        if(!empty($update_payng)){
                            ...
                            $update_payng->notes_tr         = $item['note_pays_en'];
                            $update_payng->notes_en         = $item['note_pays_tr'];
                            $update_payng->save();
                        }
                    }
                }
                if(!empty($request->new_paying)){
                    ...
                    foreach($request->new_paying as $pay){
                        $this->data['new_paying'][] = [
                            ...
                            'number_id'         => $pay['number_pays'],
                            'notes_ar'          => $pay['note_pays_ar'],
                            'notes_tr'          => $pay['note_pays_en'],
                            'notes_en'          => $pay['note_pays_tr'],
                        ];
                    }
    ...
    public function data_requests(Request $request)
    {
        ...
        ->addColumn('breef', function($model){
            return  \Illuminate\Support\Str::limit($model->description, 50, $end='...') ;
    ...
    {
        $this->data['model'] = ContactUS::findOrFail($request->model)->first();
