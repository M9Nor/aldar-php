...
@include('permissions::admin.roles.filter')
...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>  __('permissions::roles.title'),
            'items' => [
                [
        ...
        @slot('toolbar')
            ...
            <form id="massDeleteForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('RoleController@massDestroy') }}" style="display: none;">
            ...
            <form id="massRestoreForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('RoleController@massRestore') }}" style="display: none;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <input type="hidden" name="ids">
            </form>
            ...
            @if(auth()->user()->can('create', \Modules\Permissions\Entities\Role::class))
                <a href="{{ route('RoleController@create') }}" class="btn btn-outline-success btn-sm btn-icon btn-icon-md btn-elevate datatable-custom-tools" data-toggle="kt-tooltip" data-placement="right" title="" data-original-title="{{ __('cms::global.add_new') }}">
    ...
    @component('cms::components.partials.portlet')
        ...
        <table class="table table-bordered table-striped table-hover table-checkable" id="datatable">
            <thead>
                <tr class="cms-font">
                    <th></th>
                    <th>{{ __('permissions::roles.datatable.title') }}</th>
                    <th width="45%">{{ __('permissions::roles.datatable.description') }}</th>
                    <th width="20%">{{ __('permissions::roles.datatable.abilities_count') }}</th>
...
@push('scripts')
    <script>
        ...
        $(function() {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            return `<span class="kt-font-boldest" style="color: ${row.color};">${row.title}</span>`;
                        }
                    },
                    {
                        data: 'description',
                    ...
                    },
                    {
                        ...
                        orderable  : true,
                        render: function(data, type, row, meta) {
                            let output = `<div class="kt-tree"><ul>`;

                            $.each(row.permissions, function(i, v) {
                                output += `<li data-jstree='{ "icon" : "${v.icon} kt-font-info " }'><span class="kt-font-boldest kt-font-info">&nbsp;${v.title}</span><ul>`;
                                $.each(v.permissions, function(i, item) {
                                    output += `<li data-jstree='{ "type" : "file" }'>${item.title}</li>`;
                                });
                                output += `</ul></li>`;
                            });
            ...
            @if(auth()->user()->can('viewDeleted', \Modules\Permissions\Entities\Role::class))
