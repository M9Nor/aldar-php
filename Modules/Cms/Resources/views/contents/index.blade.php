...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>  __('cms::cruds.contents.contents'),
            'items' => [
                [
        ...
        @slot('toolbar')
            ...
            <form id="massDeleteForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('ContentController@massDestroy') }}" style="display: none;">
            ...
            <form id="massRestoreForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('ContentController@massRestore') }}" style="display: none;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <input type="hidden" name="ids">
            </form>
            ...
            @if(auth()->user()->can('create', Content::class))
                <a href="{{ route('ContentController@create',['type' => $type]) }}" class="btn btn-outline-success btn-sm btn-icon btn-icon-md btn-elevate datatable-custom-tools" data-toggle="kt-tooltip" data-placement="right" title="" data-original-title="{{ __('cms::global.add_new') }}">
    ...
    @component('cms::components.partials.portlet')
        ...
        <table class="table table-bordered table-striped table-hover table-checkable" id="datatable">
            <thead>
                <tr class="cms-font">
                    <th width="5%"></th>
                    @if(Content::typeHasField($type, 'image') || Content::typeHasField($type, 'title'))
                        <th width="40%">{{ Content::getTypeTitle($type, false) }}</th>
                    @endif
                    {{-- <th width="20%">{{ __('cms::cruds.contents.type.label') }}</th> --}}
                    <th width="20%">{{ __('cms::cruds.contents.languages') }}</th>
                    @if(Content::typeHasField($type, 'sort_order'))
                        <th width="10%">{{ __('cms::cruds.contents.sort_order.label') }}</th>
                    @endif
...
@push('scripts')
    <script>
        ...
        $(function() {
            $.extend(options, {
                ajax: {
                    url: "{!! route('ContentController@data',['type' => $type]) !!}",
                    type: "GET",
                    data : function (d) {
                        d._token    = '{!! csrf_token() !!}';
                ...
                columns: [
                    ...
                    @if(Content::typeHasField($type, 'image') || Content::typeHasField($type, 'title'))
                    {
                        ...
                        //     return `<span> ${row.translated_name} </span>`;
                        // }
                        render     : function(data, type, row, meta) {
                            var output = `
                                <a href="javascript:;">
            ...
            @if(auth()->user()->can('viewDeleted', Content::class, $type))
