...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>  __('cms::areas.create_new_country'),
            'items' => [
                [
                    'label' => __('cms::areas.countries'),
                    'link'  => route('CountryController@index')
                ], [
                    'label' => __('cms::areas.create_new_country'),
                    'link'  => 'javascript:;'
                ]
            ]
        ]
        ...
        @slot('toolbar')
            ...
            <div class="btn-group">
                <a href="javascript:;" data-redirect-url="{{ route('CountryController@index') }}" class="submit_form btn btn-success btn-bold">
...
@section('content')
    <form onsubmit="onFormSubmit(event);" class="kt-form kt-form--label-right" id="addNewForm" action="{{ route('CountryController@store') }}" method="POST" enctype="multipart/form-data">
        @csrf
                ...
                @component('cms::components.partials.portlet', ['portletClass' => 'kt-portlet--height-fluid'])
                    <div class="kt-section">
                        <h4 class="kt-section__title kt-section__title-sm kt-font-bolder">
                            {{__('cms::areas.create_new_country')}}
                        </h4>
