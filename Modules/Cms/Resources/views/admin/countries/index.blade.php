...
@section('subheader')
    @component('cms::includes.subheader', [
        ...
        <form id="massDeleteForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('CountryController@massDestroy') }}" style="display: none;">
        ...
        <form id="massRestoreForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('CountryController@massRestore') }}" style="display: none;">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <input type="hidden" name="ids">
        </form>
        ...
        @if(auth()->user()->can('create', Country::class))
            <a href="{{ route('CountryController@create') }}" class="btn btn-outline-success btn-sm btn-icon btn-icon-md btn-elevate datatable-custom-tools" data-toggle="kt-tooltip" data-placement="right" title="" data-original-title="{{ __('cms::global.add_new') }}">
    ...
    @component('cms::components.partials.portlet')
        ...
        <table class="table table-bordered table-striped table-hover table-checkable" id="datatable">
            <thead>
                <tr class="cms-font">
                    <th width="5%"></th>
                    <th>{{ __('cms::areas.datatable.country') }}</th>
...
@push('scripts')
    <script>
        ...
        $(function() {
            $.extend(options, {
                ajax: {
                    url: "{!! route('CountryController@data') !!}",
                    type: "GET",
                    data : function (d) {
                        d._token    = '{!! csrf_token() !!}';
...
@if(auth()->user()->can('viewDeleted', Country::class))
