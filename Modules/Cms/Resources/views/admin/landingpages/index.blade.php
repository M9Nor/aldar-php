...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>   __('cms::landing_page.landing_pages'),
            'items' => [
                [
                    'label' => __('cms::landing_page.landing_pages'),
                    'link'  => 'javascript:;'
                ],
            ]
        ]
        ...
        <form id="massDeleteForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('LandingPageController@massDestroy') }}" style="display: none;">
        ...
        <form id="massRestoreForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('LandingPageController@massRestore') }}" style="display: none;">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <input type="hidden" name="ids">
        </form>
        ...
        @if(auth()->user()->can('create', LandingPage::class))
            <a href="{{ route('LandingPageController@create') }}" class="btn btn-outline-success btn-sm btn-icon btn-icon-md btn-elevate datatable-custom-tools" data-toggle="kt-tooltip" data-placement="right" title="" data-original-title="{{ __('cms::global.add_new') }}">
        ...
        <table class="table table-bordered table-striped table-hover table-checkable" id="datatable">
            <thead>
                <tr class="cms-font">
                    ...
                    <th>{{ __('cms::landing_page.slug') }}</th>
                    {{-- <th>{{ __('cms::areas.datatable.description') }}</th> --}}
...
@push('scripts')
    <script>
        ...
        $(function() {
            $.extend(options, {
                ajax: {
                    url: "{!! route('LandingPageController@data') !!}",
                    type: "GET",
                    data : function (d) {
                        d._token    = '{!! csrf_token() !!}';
...
@if(auth()->user()->can('viewDeleted', LandingPage::class))
