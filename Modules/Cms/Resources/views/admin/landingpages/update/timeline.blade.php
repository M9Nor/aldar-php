                                        ...
                                        <div class="tab-pane {{ $loop->first ? 'active' : '' }}" id="timeline_{{ $locale }}" role="tabpanel">
                                            @include('cms::components.inputs.text', [
                                                'options' => [
                                                    ...
                                                    'value'         => old('timeline_title_' . $locale,$model->translate($locale) == NULL ? NULL : $model->translate($locale)->timeline_title),
                                                    'required'      => false,
                                                    'inline'        => false,
...
<div id="kt_repeater_2">
    @if(!empty($model->timeline))
        @foreach($model->timeline as $t)
            <div class="row" id="delete_timeline_{{$t->id}}" style="border-bottom: 1px solid #777777;padding: 20px 0;">
                <div class="col-md-4">
                    <div class="form-group kt-form__group">
                        ...
                        <div>
                            <select class="form-control" name="timeline[{{$t->id}}][language]" id="">
                                <option value="ar" {{$t->language == 'ar' ? 'selected' : '' }}>{{__('cms::landing_page.ar')}}</option>
                                <option value="en" {{$t->language == 'en' ? 'selected' : '' }}>{{__('cms::landing_page.en')}}</option>
                                <option value="fa" {{$t->language == 'fa' ? 'selected' : '' }}>{{__('cms::landing_page.fa')}}</option>
                            </select>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
                    @include('cms::components.inputs.text', [
                        'options' => [
                            'id'                => "timeline_title_{$t->id}",
                            'name'              => "timeline[{$t->id}][title]",
                            'type'              => 'text',
                ...
                <div class="col-md-4">
                    @include('cms::components.inputs.text', [
                        'options' => [
                            'id'                => "timeline_date_{$t->id}",
                            'name'              => "timeline[{$t->id}][date]",
                            'type'              => 'text',
                ...
                <div class="col-md-8">
                    @include('cms::components.inputs.textarea', [
                        'options' => [
                            'id'                => "timeline_description_{$t->id}",
                            'name'              => "timeline[{$t->id}][description]",
                            ...
                            'value'             => old('description',$t->description),
                            'required'          => true,
                            'inline'            => false,
                            'direction'         => 'ltr',
                            'status_removal'    => false,
                ...
                <div class="col-md-4">
                    @include('cms::components.inputs.text', [
                        'options' => [
                            'id'                => "timeline_sort_order_{$t->id}",
                            'name'              => "timeline[{$t->id}][sort_order]",
                            'type'              => 'text',
                ...
                <div class="col-md-4">
                    @include('cms::components.inputs.text', [
                        'options' => [
                            'id'                => "timeline_link_{$t->id}",
                            'name'              => "timeline[{$t->id}][link]",
                            'type'              => 'text',
                ...
                <div class="col-md-4">
                    @php
                        if($t->icon == NULL){
                            $path = Timeline::getIconImage(new Timeline, '100x100');
                        }else{
                            $path = Timeline::getIconImage($t, '100x100');
                        }
                    @endphp
                    <img style="width: 50px" src="{{$path}}" alt="">
                    <div class="form-group kt-form__group">
                        ...
                        <div>
                            <input type="file" name="timeline[{{$t->id}}][icon]" id="">
                        </div>
                ...
                <div class="col-md-4">
                    @php
                        if($t->image == NULL){
                            $path = Timeline::getImage(new Timeline, '100x100');
                        }else{
                            $path = Timeline::getImage($t, '100x100');
                        }
                    @endphp
                    <img style="width: 50px" src="{{$path}}" alt="">
                    <div class="form-group kt-form__group">
                        ...
                        <div>
                            <input type="file" name="timeline[{{$t->id}}][image]" id="">
                        </div>
                ...
                <div class="col-md-12" style="margin-bottom: 10px">
                    <a href="javascript:;" data-timelineId="{{$t->id}}" class="btn-sm btn btn-label-danger btn-bold delete-timeline">
    ...
    @endif
    <div class=" row">
        <div data-repeater-list="new_timeline" class="col-md-12">
