...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>  __('cms::users.create_new_user'),
            'items' => [
                [
                    'label' => __('cms::users.title'),
                    'link'  => route('UserController@index')
                ], [
                    'label' => __('cms::users.create_new_user'),
                    'link'  => 'javascript:;'
                ]
            ]
        ]
...
@section('content')
    <div class="kt-portlet kt-portlet--tabs kt-portlet--last kt-portlet--responsive-mobile" id="kt_page_portlet">
        ...
        <div class="kt-portlet__body">
            <form onsubmit="onFormSubmit(event);" class="kt-form" id="addNewForm" action="{{ route('UserController@store') }}" method="POST" enctype="multipart/form-data">
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="add_tab_1" role="tabpanel">
                        @include('cms::admin.users.sections.account')
                    </div>
                    <div class="tab-pane" id="add_tab_2" role="tabpanel">
                        @include('cms::admin.users.sections.profile')
                    </div>
                </div>
            </form>
        </div>
        ...
        $('[name="username"], [name="email"]').on('input', function() {
            ...
            if(keyword.length > 2)
            {
                ...
                $.ajax({
                    ...
                    url: "{{ route('UserController@validateIdentity') }}",
                    type: 'GET',
