...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>  __('cms::users.myprofile'),
            'items' => [
                [
                    'label' => __('cms::users.home'),
                    'link'  => route('DashboardController@index')
                ], [
                    'label' => __('cms::users.myprofile'),
                    'link'  => 'javascript:;'
                ]
            ]
        ]
...
@section('content')
    <div class="kt-portlet kt-portlet--tabs kt-portlet--last kt-portlet--responsive-mobile" id="kt_page_portlet">
        ...
        <div class="kt-portlet__body">
            <form onsubmit="onFormSubmit(event);" class="kt-form" id="addNewForm" action="{{ route('UserController@update_profile',['model'=>$model->id]) }}" method="POST" enctype="multipart/form-data">
