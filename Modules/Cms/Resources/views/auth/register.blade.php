...
@section('title', __('cms::auth.register.title'))

@section('content')

<div class="kt-login__signup d-block">
    <div class="kt-login__head">
        <h3 class="kt-login__title">{{ __('cms::auth.register.title') }}</h3>
    </div>
    <form onsubmit="onFormSubmit(event);" class="kt-form" method="POST" action="{{ route('register') }}">
        @csrf
        <div class="input-group">
            <input id="username" type="text" class="form-control" name="username" placeholder="{{ __('cms::auth.attributes.username.placeholder') }}" value="{{ old('name') }}" autocomplete="name" autofocus>
        </div>
        <div class="input-group">
        ...
        <div class="kt-login__actions">
            <button type="submit" class="btn btn-brand btn-pill kt-login__btn-primary" data-on-loading-text="{{ __('cms::auth.register.registering') }}">{{ __('cms::auth.register.create_account') }}</button>&nbsp;&nbsp;
