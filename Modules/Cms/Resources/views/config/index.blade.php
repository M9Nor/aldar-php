...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>  __('cms::cruds.config.config'),
            'items' => [
                [
        ...
        @slot('toolbar')
            ...
            <form id="massDeleteForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('ConfigController@massDestroy') }}" style="display: none;">
            ...
            <form id="massRestoreForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('ConfigController@massRestore') }}" style="display: none;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <input type="hidden" name="ids">
            </form>
            ...
            @if(auth()->user()->can('create', \Modules\Cms\Entities\Config::class))
                <a href="{{ route('ConfigController@create') }}" class="btn btn-outline-success btn-sm btn-icon btn-icon-md btn-elevate datatable-custom-tools" data-toggle="kt-tooltip" data-placement="right" title="" data-original-title="{{ __('cms::global.add_new') }}">
    ...
    @component('cms::components.partials.portlet')
        ...
        <table class="table table-bordered table-striped table-hover table-checkable" id="datatable">
            <thead>
                <tr class="cms-font">
                    <th></th>
                    <th width="35%">{{ __('cms::cruds.config.label.label') }}</th>
                    <th width="10%">{{ __('cms::cruds.config.key.label') }}</th>
                    <th width="30%">{{ __('cms::cruds.config.val.label') }}</th>
                    <th width="15%">{{ __('cms::cruds.config.languages') }}</th>
...
@push('scripts')
    <script>
        ...
        $(function() {
            $.extend(options, {
                ajax: {
                    url: "{!! route('ConfigController@data') !!}",
                    type: "GET",
                    data : function (d) {
                        d._token    = '{!! csrf_token() !!}';
                ...
                columns: [
                    ...
                    //         return `<img class="img-rounded product-list-images" src="${row.image.src}">`;
                    //     }
                    // },
                    {
                        data: 'new_title',
                        ...
                        orderable  : true,
                        render: function(data, type, row, meta) {
                            return `<span style="font-weight:bold">${row.translated_name}</span>`;
                        }
                    },
                    {
                        data: 'key',
                        ...
                        orderable  : true,
                        render: function(data, type, row, meta) {
                            return `<span style="font-weight:bold"> ${row.key} </span>`;
                        }
                    },
                    {
                        data: 'val',
                        ...
                        orderable  : false,
                        render: function(data, type, row, meta) {
                            return `<span style="font-weight:bold"> ${row.val} </span>`;
                        }
                    },
                    {
                        data       : "languages",
            ...
            @if(auth()->user()->can('viewDeleted', \Modules\Cms\Entities\Config::class))
