...
@section('subheader')
    @component('cms::includes.subheader', [
        'options' => [
            'title' =>  __('cms::cruds.categories.categories'),
            'items' => [
                [
        ...
        @slot('toolbar')
            ...
            <form id="massDeleteForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('CategoryController@massDestroy') }}" style="display: none;">
            ...
            <form id="massRestoreForm" onsubmit="onFormSubmit(event);" method="POST" action="{{ route('CategoryController@massRestore') }}" style="display: none;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <input type="hidden" name="ids">
            </form>
            ...
            @if(auth()->user()->can('create', [Category::class,$type]))
                <a href="{{ route('CategoryController@create',['type' => $type ?? '']) }}" class="btn btn-outline-success btn-sm btn-icon btn-icon-md btn-elevate datatable-custom-tools" data-toggle="kt-tooltip" data-placement="right" title="" data-original-title="{{ __('cms::global.add_new') }}">
    ...
    @component('cms::components.partials.portlet')
        ...
        <table class="table table-bordered table-striped table-hover table-checkable" id="datatable">
            <thead>
                <tr class="cms-font">
                    <th></th>
                    @if(Category::typeHasField($type ?? '', 'image') || Category::typeHasField($type ?? '', 'title'))
                        <th width="35%">{{ __('cms::cruds.categories.title.label') }}</th>
                    @endif
                    {{-- <th width="10%">{{ __('cms::cruds.categories.type.label') }}</th> --}}
                    @if(Category::typeHasField($type ?? '', 'parent'))
                        <th width="30%">{{ __('cms::cruds.categories.parent.label') }}</th>
                    @endif
                    <th width="15%">{{ __('cms::cruds.categories.languages') }}</th>
                    @if(Category::typeHasField($type ?? '', 'sort_order'))
                        <th width="5%">{{ __('cms::cruds.categories.sort_order.label') }}</th>
                    @endif
...
@push('scripts')
    <script>
        ...
        $(function() {
            $.extend(options, {
                ajax: {
                    url: "{!! route('CategoryController@data',['type' => $type ?? '']) !!}",
                    type: "GET",
                    data : function (d) {
                        d._token    = '{!! csrf_token() !!}';
                ...
                columns: [
                                    ...
                                    <div class="kt-user-card-v2">
                                        @if(Category::typeHasField($type ?? '', 'image'))
                                            <div class="kt-user-card-v2__pic custom">
                                                <img src="${row.image.src}" alt="photo">
                                            </div>
                                        @endif
                                        @if(Category::typeHasField($type ?? '', 'title'))
                                            <div class="kt-user-card-v2__details">
                                                <span class="kt-user-card-v2__name">${row.new_title}</span>
                                            </div>
                                        @endif
                                    </div>
                                </a>
                    ...
                    //     orderable  : true,
                    //     render: function(data, type, row, meta) {
                    //         return `<span style="font-weight:bold"> ${row.type} </span>`;
                    //     }
                    // },
                    @if(Category::typeHasField($type ?? '', 'parent'))
                    {
                        ...
                        orderable  : false,
                        render: function(data, type, row, meta) {
                            return `<span style="font-weight:bold"> ${row.parent_categories} </span>`;
                        }
                    },
                    @endif
                    {
                    ...
                    @if(Category::typeHasField($type ?? '', 'sort_order'))
                    {
                        ...
                        render: function(data, type, row, meta) {
                            if(row.sort_order != null){
                                return `<span style="font-weight:bold"> ${row.sort_order} </span>`;
                            }else{
                                return ``;
                            }
                        }
            ...
            @if(auth()->user()->can('viewDeleted', [Category::class, $type] ))
