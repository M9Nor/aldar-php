...
@section('subheader')
    @component('cms::includes.subheader', [
        ...
        @slot('toolbar')
            ...
            <div class="btn-group">
                <a href="javascript:;" data-redirect-url="{{ route('CategoryController@index',['type' => $type]) }}" class="submit_form btn btn-success btn-bold">
...
@section('content')
    <form onsubmit="onFormSubmit(event);" class="kt-form kt-form--label-right" id="addNewForm" action="{{ route('CategoryController@store',['type' => $type]) }}" method="POST" enctype="multipart/form-data">
        @csrf
                                ...
                                <div class="col-sm-12">
                                    @include('cms::components.inputs.select2', [
                                        'options' => [
                                            ...
                                            'multiple'          => false,
                                            'clear_button'      => true,
                                            'url'               => route('CategoryController@getCategoriesSelect2',['type' => $type == 'filters' ? 'filter_categories' : $type]),
                                            'items_per_page'    => 20,
                                            'dir'               => 'rtl',
                                ...
                                @foreach($supportedLangs as $locale => $properties)
                                    <div class="tab-pane {{ $loop->first ? 'active' : '' }}" id="tab_{{ $locale }}" role="tabpanel">
                                        <div class="row">
                                                ...
                                                <div class="col-sm-12">
                                                    @include('cms::components.inputs.image', [
                                                        'options' => [
                                                            ...
                                                            'default'       => old('image_'.$locale, Category::getImage(new Category, $previewSize[0].'x'.$previewSize[1], $type)),
                                            ...
                                            @endif
                                            @if(Category::typeHasField($type, 'brief'))
                                                <div class="col-sm-12">
                                        ...
                                        @if(Category::typeHasField($type, 'icon_image'))
                                            @include('cms::components.inputs.image', [
                                                'options' => [
                                                    ...
                                                    'default'       => old('icon_image', Category::getIconImage(new Category, $previewSize[0].'x'.$previewSize[1], $type)),
                                                    'required'      => Category::isFieldRequired($type, 'icon_image'),
                                                    // 'inline'        => '3:9',
                                                    'browse'        => __('cms::cruds.contents.icon_image.add_text'),
                                                    'remove'        => __('cms::cruds.contents.icon_image.remove_text'),
                                                    'width'         => $previewSize[0].'px',
                                                    'height'        => $previewSize[1].'px',
                                                ]
                                            ])
