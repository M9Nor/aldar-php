...
| H4 | **Unbounded image generation.** `Modules/Cms/Http/Controllers/ImageController.php` `show()` accepts any `{size}`, and `quality`, `extension` and `mark` query parameters, then writes a cached variant per combination (the cache is at 11,788 files, 727 MB). Visitors can fill the disk. | Allow only the sizes actually used by views and entities (e.g. `85x85`, `1000x750`, `500x375`, `400x400`, `698x500`, `150x150`, `1920x1280` …). The final list is generated by grepping views and entities during implementation, plus `original`. Unknown sizes return 404. Ignore `quality`, `extension` and `mark`, which no view uses. |
| H5 | **Spam on the contact forms.** 1,780 of 3,700 leads contain URLs, and 20 contain HTML tags. There is no rate limit on `contact-us/store`, `store-inner` or `subscribe`. | Add a hidden honeypot field to the three forms. A filled honeypot gets the normal success JSON but nothing is stored. Throttle each endpoint to 5 requests per 10 minutes per IP; the 429 response is JSON with a translated message. Humans see no change. |
...
|---|---|---|
| N1 | Active ROOT account `developer` (`root@namaa-solutions.com`, id 1). Its seeder password was a literal in `Modules/Cms/Config/config.php`, and the login flow does not check `disabled_at`. | Revoke all role assignments. Set a random 64-character password that is never stored or displayed. Set `disabled_at` and soft-delete the account. A test proves login is rejected. |
...
| N7 | SSH keys on the Hostinger account. Five keys exist and none belong to the vendor. | **Owner action:** confirm both the two keys whose comments are near-identical spellings of the same personal Gmail address. Recommended: enable 2FA on hPanel, because the account hosts 16 sites. |
...
- **Playwright project** at `tests/e2e/`, TypeScript. Phase H adds `tests/e2e/security/hotfix.spec.ts`; Phase 0 extends the same project.
- **Security tests.** Each finding gets a test that fails on the current code and passes after the fix:
  ...
  - H2, H3: both routes redirect anonymous users to login; the dashboard button still updates currencies.
  - H4: an unknown size returns 404, and every whitelisted size returns 200.
  - H5: a filled honeypot stores no row, and the sixth request within 10 minutes returns 429.
...
1. Resolve `git diff --name-only origin/main...<git-ref>` into a file list (additions and modifications only).
2. On the server, back up those paths into `~/aldar-backup/hotfix-<timestamp>.tar.gz`. Dump the DB through Laravel's own config, as `reference_aldar_emlak_hosting` describes.
3. Upload the files with `rsync --files-from` from a `git archive` of the ref.
4. Run `php artisan view:clear` and `cache:clear` with `/opt/alt/php74/usr/bin/php`.
5. Run a read-only post-deploy check against https://aldar-emlak.com:
   - public pages return 200 in ar/en;
   - `/update_currency` and `/clear-cache` redirect to login;
